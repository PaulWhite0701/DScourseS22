...
library(nnet)
library(kernlab)
install.packages("magrittr","modelsummary","rpart",
                 "e1071","kknn","nnet","kernlab")
set.seed(100)
...
tree_parm_df1 <- tibble(cost_complexity = seq(.001,.2,by=.0005))
...
tree_wf<-workflow() %>% add_model(tune_tree_spec) %>%
  add_formula(high.earner ~ education + marital.status + race + workclass + occupation + relationship + sex + age + capital.gain + capital.loss + hours)
